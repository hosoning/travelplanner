...
let fbApp, db, auth, googleProvider;
if(fbReady()){
  try{
...
function applyTheme(key){
  ...
  document.getElementById('wp').style.background = theme.bg;
  document.documentElement.style.setProperty('--theme-accent', theme.accent);
  localStorage.setItem('theme', key);
  S.theme = key;
}

// ── XHS POOL (30 keywords — rotate on refresh) ───────────────
const XHS_POOL = [
  { kw:'首尔美食推荐 弘大',   title:'弘大必吃清单',       desc:'炸鸡+年糕+血肠，本地人最爱的夜宵路线' },
  { kw:'首尔购物攻略 东大门', title:'东大门怎么买才不后悔', desc:'批发价拿货技巧，最佳扫货时间表' },
  ...
  { kw:'韩国特产伴手礼',       title:'回国必买伴手礼清单',   desc:'不到1000元预算装满行李箱方案' },
  ...
  { kw:'韩国整形医院 参考',    title:'韩国整形注意事项',     desc:'正规机构选择+价格参考+恢复期建议' },
  { kw:'首尔新村大学路',       title:'新村大学街文艺路线',   desc:'独立书店+复古唱片店+小剧场一日游' },
  { kw:'韩国高铁KTX攻略',      title:'KTX高铁订票指南',      desc:'提前几天买最便宜？App订票教程' },
  { kw:'首尔乐天世界 攻略',    title:'乐天世界省钱指南',     desc:'门票优惠+人少时段+必玩项目排名' },
...
function getXHSRecs(){
  var pool = XHS_POOL;
  var start = _xhsOffset % pool.length;
  var result = [];
  for(var i=0; i<6; i++) result.push(pool[(start+i) % pool.length]);
  return result;
}
window.refreshXHS = function(){
  _xhsOffset = (_xhsOffset + 6) % XHS_POOL.length;
  renderHome();
};
...
function ic(n,sz){ var s=IC[n]||IC.plus; var size=sz||22; return s.replace('<svg ','<svg width="'+size+'" height="'+size+'" '); }
...
function renderMentions(text){
  ...
  Object.entries(S.members).forEach(function(entry){
    ...
    try{
      escaped = escaped.replace(new RegExp(escHtml(safeName),'g'), span);
    }catch(e){}
  });
  return escaped;
}
...
function initAuth(){
  ...
  onAuthStateChanged(auth, function(user){
    if(user){
      S.uid=user.uid; S.userEmail=user.email; S.userPhoto=user.photoURL;
      console.info('[Auth] Signed in:', user.email);
    } else {
      S.uid=null; S.userEmail=null; S.userPhoto=null;
    }
    var setV = $('#v-set'); if(setV&&setV.classList.contains('active')) renderSet();
  });
}
...
async function fbCreateTrip(code,name){
  var mid='u_'+Date.now(); var color=COLORS[0]; var members={}; members[mid]={name:name,color:color};
...
function sysPrompt(){
  ...
  return 'You are Travoo Butler, a smart travel assistant for the trip "'+((S.trip&&S.trip.name)||'Trip')+'".\nToday: '+today()+(td?' - '+td.title:'')+'. Members: '+Object.values(S.members).map(function(m){ return m.name; }).join(', ')+'.\nReply in the user\'s language. Be concise and practical.';
}
...
function extractDate(str){
  var wds=['日','一','二','三','四','五','六']; var year=new Date().getFullYear();
...
// Fix #3: Multilingual keyword detection (zh-CN, zh-TW, Cantonese, EN)
function guessType(text){
  var t2=text||'';
  // Food
  if(/早餐|午餐|晚餐|早飯|午飯|晚飯|食飯|吃飯|吃饭|餐廳|餐厅|小吃|美食|燒麥|烧麦|手把肉|涮鍋|火鍋|火锅|咖啡|飲食|食嘢|搵嘢食|吃|eat|breakfast|lunch|dinner|restaurant|food|meal|cafe/i.test(t2)) return 'food';
  // Checkin
  if(/入住|抵達|抵达|到達|到达|接機|接机|check.?in|チェックイン|酒店|民宿|賓館|宾馆/i.test(t2)) return 'checkin';
  // Transport
  if(/打的|打車|打车|搭車|搭车|的士|滴滴|高鐵|高铁|火車|火车|飛機|飞机|地鐵|地铁|巴士|公交|包車|包车|出發|出发|返程|離開|离开|乘車|乘车|taxi|train|flight|bus|uber|transport/i.test(t2)) return 'transport';
  // Attraction
  if(/遊覧|游览|參觀|参观|景區|景区|景點|景点|博物館|博物馆|公園|公园|古城|寺廟|寺庙|長城|长城|草原|沙漠|火山|海灘|海滩|beach|museum|park|temple|attraction|sightseeing/i.test(t2)) return 'attr';
  // Activity
  if(/騎馬|骑马|滑沙|體驗|体验|活動|活动|徒步|漂流|攀岩|表演|hiking|activity|experience/i.test(t2)) return 'act';
  // Rest
  if(/休息|休整|自由|購物|购物|shopping|free|rest/i.test(t2)) return 'rest';
  return 'leisure';
}
function guessApps(text){
  var apps=[]; var t2=text||'';
  if(/打的|打車|打车|搭的|滴滴|taxi|uber/i.test(t2)) apps.push('didi');
  if(/景區|景點|景区|景点|參觀|游览|導航|地圖|地图|navigation|map/i.test(t2)) apps.push('maps');
...
function parseTableFormat(lines){
  var days={},dayOrder=[];
  var headerIdx=-1; var colMap={date:0,time:1,title:2,transport:3,spend:4,lodge:5,bag:6,notes:7};
  for(var i=0;i<Math.min(lines.length,5);i++){
    var cells=lines[i].split('\t'); var str=cells.join(' ');
  ...
  var currentDate=null; var startRow=headerIdx>=0?headerIdx+1:0;
  for(var i=startRow;i<lines.length;i++){
    ...
    var titleCell=(cells[colMap.title!=null?colMap.title:2]||'').trim(); if(!titleCell) continue;
    var timeCell=(cells[colMap.time!=null?colMap.time:1]||'').trim(); var transCell=(cells[colMap.transport!=null?colMap.transport:3]||'').trim(); var spendCell=(cells[colMap.spend!=null?colMap.spend:4]||'').trim(); var lodgeCell=(cells[colMap.lodge!=null?colMap.lodge:5]||'').trim(); var bagCell=(cells[colMap.bag!=null?colMap.bag:6]||'').trim(); var notesCell=(cells[colMap.notes!=null?colMap.notes:7]||'').trim();
    ...
    var dayData=days[currentDate.date]; var combined=titleCell+' '+transCell;
    dayData.items.push({id:currentDate.date.replace(/-/g,'')+'_'+(dayData.items.length+1),time:timeCell||'全天',title:titleCell,transport:transCell,sMin:sp.sMin,sMax:sp.sMax,lodge:lodgeCell,bag:bagCell,notes:notesCell,apps:guessApps(combined),type:guessType(combined),hi:/高铁|包车|飞机/.test(transCell),urgent:/准时/.test(notesCell)});
  }
...
function parseFreeText(lines){
  ...
  lines.forEach(function(line){
    ...
    var title=line.substring(0,40); var dayData=days[currentDate.date];
    dayData.items.push({id:currentDate.date.replace(/-/g,'')+'_'+(dayData.items.length+1),time:time,title:title,transport:'',sMin:null,sMax:null,lodge:'',bag:'',notes:'',apps:guessApps(title),type:guessType(title),hi:/高铁|包车|飞机/.test(title),urgent:false});
  });
...
function _updateTripDates(days){
  ...
  var first=days[0],last=days[days.length-1]; var autoDates=first.month+'/'+first.day+' — '+last.month+'/'+last.day;
...
function calcSettle(){
  ...
  S.expenses.forEach(function(e){ var amt=Number(e.amount)||0; var split=e.splitAmong||ids; var share=amt/split.length; if(bal[e.paidBy]!==undefined) bal[e.paidBy]+=amt; split.forEach(function(id){ if(bal[id]!==undefined) bal[id]-=share; }); });
  var txns=[]; var deb=ids.filter(function(id){ return bal[id]<-0.01; }).map(function(id){ return {id:id,a:-bal[id]}; }).sort(function(a,b){ return b.a-a.a; }); var crd=ids.filter(function(id){ return bal[id]>0.01; }).map(function(id){ return {id:id,a:bal[id]}; }).sort(function(a,b){ return b.a-a.a; });
  var di=0,ci=0;
...
function checkNotifs(){
  ...
  var now=new Date(); var shown=JSON.parse(localStorage.getItem('shownNotifs')||'[]');
  todayDay.items.forEach(function(item){
    if(!item.time||item.time==='全天') return;
    var parts=(item.time+':00').split(':'); var h=parseInt(parts[0]),m=parseInt(parts[1]);
...
function getSmartTip(item){
  ...
  // Fix #9: Travel Documents instead of ID card
  if(item.transport&&/高铁|高鐵|KTX|train|flight|飞机|飛機/.test(item.transport)) return t('travelDocs')+'、出行前请提前20分钟到场';
...
function startVoice(onResult){
  ...
  ov.innerHTML='<div class="voice-ring" id="vring">'+ic('mic',40)+'</div><div class="voice-hint" id="vhint">'+t('listening')+'</div><div class="voice-text" id="voice-txt"></div><div style="display:flex;gap:12px;margin-top:28px"><div class="voice-cancel" id="v-done" style="background:rgba(255,255,255,.18);color:#fff;font-weight:700;padding:10px 28px">完成</div><div class="voice-cancel" id="v-cancel">'+t('cancel')+'</div></div>';
...
// Fix #3: multilingual voice intent
...
window.switchTab=function(name){
  $$('.tab').forEach(function(t){ t.classList.remove('on'); }); $$('.view').forEach(function(v){ v.classList.remove('active'); });
  ...
  if(fn[name]) fn[name]();
  var micFab=document.getElementById('gfab-mic'); var addFab=document.getElementById('gfab-add');
  if(micFab){ if(name==='home'){ micFab.removeAttribute('hidden'); }else{ micFab.setAttribute('hidden',''); } }
  if(addFab){ if(name==='exp'){ addFab.removeAttribute('hidden'); }else{ addFab.setAttribute('hidden',''); } }
};
...
function renderOnboarding(){
  ...
  var langChips=['zh-CN','zh-TW','en'].map(function(l){ return '<div class="chip '+(S.lang===l?'on':'')+'" style="padding:5px 14px;font-size:13px;font-weight:600" onclick="setLang(\''+l+'\')">'+LANG_LABEL[l]+'</div>'; }).join('');
...
window.handleJoin=async function(){
  var code=($('#ob-code')&&$('#ob-code').value.trim().toUpperCase())||''; var name=($('#ob-name')&&$('#ob-name').value.trim())||'';
...
function renderTripList(){
  var cards=S.localTrips.map(function(tr){
    return '<div class="tc" onclick="enterTrip(\''+tr.code+'\')"><div class="tc-bg"></div><div class="tc-body"><div class="tc-name">'+escHtml(tr.name||'我的旅行')+'</div><div class="tc-date">'+escHtml(tr.dates||'')+'</div><div class="tc-code">'+escHtml(tr.code)+'</div></div></div>';
  }).join('');
  document.getElementById('app').innerHTML=
...
function renderHome(){
  ...
  // Smart recs
  var recs=buildSmartRecs(todayDay,h); var recsHtml='';
  if(recs.length){
  ...
  xhsRecs.forEach(function(r){
    ...
    xhsHtml+='<div class="xhs-card" onclick="openXHS(\''+kwEnc+'\')">'+
      ...
      '<div class="xhs-body"><div class="xhs-ttl">'+escHtml(r.title)+'</div><div class="xhs-desc">'+escHtml(r.desc)+'</div></div></div>';
  });
  xhsHtml+='</div></div>';
  ...
  if(todayDay&&todayDay.items.length){
    ...
    todayDay.items.forEach(function(item){
      ...
      bottomHtml+='<div class="lr" onclick="showActDetail(\''+item.id+'\')">'+
        ...
        '<div style="flex:1"><div style="font-size:15px;font-weight:600">'+renderMentions(item.title)+'</div>'+spHtml+'</div>'+urgDot+'</div>';
    });
  ...
  // Fix #10: Not Planned when no days
  var heroDay,heroTitle;
  if(days.length===0){
    heroDay=t('notPlanned'); heroTitle=trip.name||'Travoo';
  } else if(todayDay){
    heroDay=getWdLabel(todayDay.wd)+' · '+t('today'); heroTitle=todayDay.title;
  } else if(nowDate<startDate){
    heroDay=t('countdown'); heroTitle=trip.name||'Travoo';
  } else {
    heroDay=t('tripEnded'); heroTitle=trip.name||'Travoo';
  }

  // Fix #4 hero member row
  var memberEntries=Object.entries(S.members); var avatarsHtml='';
  memberEntries.slice(0,5).forEach(function(entry){
    var id=entry[0],m=entry[1]; var img=memberAvatar(id);
    if(img) avatarsHtml+='<div class="hav"><img src="'+img+'" alt="'+escHtml(m.name)+'"></div>';
    else avatarsHtml+='<div class="hav" style="background:'+m.color+'">'+((m.name||'?')[0])+'</div>';
  });
  ...
  var memberRow=memberEntries.length>0?
    '<div class="hero-members"><div style="display:flex">'+avatarsHtml+'</div><span class="hero-mem-info">'+memberEntries.length+' '+t('nMembers')+'</span><div class="hero-share-btn" onclick="showTripCodeModal()">'+ic('share',13)+' '+t('invite')+'</div></div>':'';
...
function buildSmartRecs(todayDay,h){
  ...
  if(h>=7&&h<=9) recs.push({type:'早餐推荐',title:'附近早餐',desc:'大众点评搜附近早餐，查看4.5+评分',action:"openApp('dianping')"});
  if(h>=11&&h<=13) recs.push({type:'午餐推荐',title:'当地特色午餐',desc:'大众点评搜附近好评餐厅',action:"openApp('dianping')"});
...
function renderItin(){
  ...
  days.forEach(function(day,di){
    ...
    // Fix #4: editable day title
    pagesHtml+='<div class="itin-page" id="ipg-'+di+'">'+
      '<div class="day-hdr">'+
        '<div class="day-hdr-title" onclick="editDayTitle('+di+')" title="'+t('editDayTitle')+'">'+escHtml(day.title)+' '+ic('edit',11)+'</div>'+
  ...
  var emptyItin=days.length===0?
    '<div class="empty" style="min-height:60vh">'+ic('cal',52)+'<div class="empty-ttl">'+t('notPlanned')+'</div><div class="empty-sub">点击右上角「+」导入或添加行程</div></div>':'';
  ...
  if(sl){
    sl.addEventListener('scroll',function(){ var idx=Math.round(sl.scrollLeft/sl.offsetWidth); if(idx!==_itinDay){ _itinDay=idx; $$('.dtab').forEach(function(d,i){ d.classList.toggle('on',i===idx); }); var tab=$('#dtab-'+idx); if(tab) tab.scrollIntoView({behavior:'smooth',block:'nearest',inline:'center'}); } },{passive:true});
  }
}

// Fix #4: edit day title inline
window.editDayTitle=function(di){
  var days=getDays(); if(!days[di]) return;
  showModal(
    '<div class="sh"></div>'+
    '<div style="font-size:18px;font-weight:700;margin-bottom:14px">'+t('editDayTitle')+'</div>'+
    '<input class="inp" id="edt-title" value="'+escHtml(days[di].title)+'" style="margin-bottom:14px">'+
    '<button class="btn btn-p btn-full" onclick="submitEditDayTitle('+di+')">'+t('save')+'</button>'
  );
};
window.submitEditDayTitle=async function(di){
  var title=$('#edt-title')&&$('#edt-title').value.trim(); if(!title) return;
  var days=JSON.parse(JSON.stringify(getDays())); days[di].title=title;
  closeModal(); showLoad(); await fbSaveDays(days); hideLoad(); renderItin();
};

function renderActCard(item){
  var spend=spendStr(item); var isHi=item.hi&&item.transport;
  var chipsHtml='';
  ...
  return '<div class="act'+(item.urgent?' urgent':'')+'" onclick="showActDetail(\''+item.id+'\')">'+
    ...
    '<div class="act-body">'+
      '<div class="act-title">'+renderMentions(item.title)+'</div>'+  // Fix #5: mentions
...
window.showEditItemModal=function(id){
  ...
  showModal(
    '<div class="sh"></div><div style="font-size:18px;font-weight:700;margin-bottom:14px">'+t('editItem')+'</div>'+
...
// Fix #5: placeholders use generic terms, not names
window.showAddItemModal=function(dayIdx){
  showModal(
    '<div class="sh"></div><div style="font-size:18px;font-weight:700;margin-bottom:14px">'+t('addItem')+'</div>'+
...
window.showAddDayModal=function(){
  showModal(
    '<div class="sh"></div><div style="font-size:18px;font-weight:700;margin-bottom:14px">'+t('addNewDay')+'</div>'+
...
window.showTripEditModal=function(){
  var trip=S.trip||{}; var hasCfg=!!(S.aiConfig.apiKey&&S.aiConfig.endpoint);
  showModal(
    '<div class="sh"></div><div style="font-size:18px;font-weight:700;margin-bottom:14px">'+t('tripInfoTitle')+'</div>'+
    ...
    '<div style="padding:14px;background:var(--g1);border:1px solid var(--gb);border-radius:var(--r2);margin-bottom:14px">'+
      ...
      '<button class="btn btn-g btn-full" style="padding:11px'+(hasCfg?';margin-bottom:8px':'')+'" onclick="showPasteImport()">'+ic('edit',16)+' '+t('pasteImport')+'</button>'+
...
window.submitPasteImport=async function(){
  ...
  try{
    ...
    if(!days||!days.length) throw new Error('未识别到行程数据。请确认格式包含日期（如 2000/1/1）');
...
function renderExp(){
  ...
  v.innerHTML=
    '<div class="nav"><div class="nav-title">'+t('exp')+'</div></div>'+
...
window.switchExpTab=function(tab,el){ $$('.ptab').forEach(function(t){ t.classList.remove('on'); }); el.classList.add('on'); var lp=$('#exp-list-pane'),sp=$('#exp-settle-pane'); if(lp) lp.style.display=tab==='list'?'block':'none'; if(sp) sp.style.display=tab==='settle'?'block':'none'; if(tab==='settle') renderSettle(); };
...
function refreshExpList(){
  ...
  sum.innerHTML='<div class="exp-sum"><div class="estat"><div class="estat-lbl">'+t('total')+'</div><div class="estat-val" style="color:var(--red)">¥'+tot.toFixed(0)+'</div></div><div class="estat"><div class="estat-lbl">'+t('myPaid')+'</div><div class="estat-val" style="color:var(--orange)">¥'+myP.toFixed(0)+'</div></div><div class="estat"><div class="estat-lbl">'+t('cnt')+'</div><div class="estat-val">'+S.expenses.length+'</div></div></div>';
...
function renderSettle(){
  ...
  el.innerHTML=txns.map(function(tx){ return '<div class="srow"><div class="srow-from"><div class="srow-name">'+escHtml(memberName(tx.from))+'</div><div class="srow-to">'+t('transferTo')+' '+escHtml(memberName(tx.to))+'</div></div><div class="srow-amt">¥'+tx.amount.toFixed(2)+'</div></div>'; }).join('');
}
window.showExpDetail=function(id){
  ...
  showModal('<div class="sh"></div><div style="font-size:20px;font-weight:700;margin-bottom:4px">'+escHtml(e.description||t('other'))+'</div><div style="font-size:36px;font-weight:800;color:var(--red);margin:10px 0">¥'+Number(e.amount).toFixed(2)+'</div><div class="list" style="margin-bottom:14px"><div class="lr" style="cursor:default"><span class="lr-lbl">'+t('paidBy')+'</span><span class="lr-val">'+escHtml(memberName(e.paidBy))+'</span></div><div class="lr" style="cursor:default"><span class="lr-lbl">'+t('splitW')+'</span><span class="lr-val">'+escHtml(splitNames)+'</span></div><div class="lr" style="cursor:default"><span class="lr-lbl">'+t('date')+'</span><span class="lr-val">'+escHtml(e.date||'')+'</span></div></div><button class="btn btn-d btn-full" onclick="fbDelExpense(\''+e.id+'\');closeModal();toast(t(\'deleted\'))">'+ic('trash',16)+' '+t('del')+'</button>');
};
window.showAddExpenseModal=function(prefill){
  ...
  showModal(
    '<div class="sh"></div><div style="font-size:18px;font-weight:700;margin-bottom:14px">'+t('addExpense')+'</div>'+
    ...
    '<div class="inp-lbl">'+t('amount')+'</div><input class="inp" id="ex-amt" type="number" placeholder="0.00" value="'+(prefill.amount!=null?String(prefill.amount):'')+'" style="margin-bottom:10px;font-size:22px;font-weight:700">'+
...
window.captureReceipt=function(){
  ...
  inp.onchange=async function(){
    ...
    rd.onload=async function(e){
      ...
      if(r){ var ae=$('#ex-amt'); if(ae&&r.amount) ae.value=r.amount; var de=$('#ex-desc'); if(de&&r.description) de.value=r.description; if(r.category) $$('#cat-chips .chip').forEach(function(c){ c.classList.toggle('on',c.dataset.c===r.category); }); toast(t('recognizeOk')); }
...
function renderChat(){
  ...
  v.innerHTML=
    // Fix #13: spacer on left for true centering; Fix #7: Butler name
...
function renderMsg(m){ var isU=m.role==='user'; var time=''; if(m.ts&&m.ts.toDate) time=m.ts.toDate().toLocaleTimeString('zh',{hour:'2-digit',minute:'2-digit'}); var metaHtml=time?'<div class="mmeta">'+time+'</div>':''; return '<div class="msg '+(isU?'msg-u':'msg-a')+'"><div class="mbubble">'+(m.content||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>')+'</div>'+metaHtml+'</div>'; }
...
window.sendChatMsg=async function(forceTxt){
  ...
  var sugWrap=$('#csug-wrap'); if(sugWrap) sugWrap.style.display='none';
  try{
...
window.showAIConfig=function(){
  var cfg=S.aiConfig; var pct=Math.min(100,(S.tokenUsed/Math.max(S.tokenBudget*100,1))*100); var fillColor=pct>80?'var(--red)':'var(--green)';
  showModal(
    '<div class="sh"></div>'+
    // Fix #12: just t('aiCfg'), no extra "AI" prefix
    ...
    '<div style="display:flex;align-items:center;gap:10px;margin-bottom:14px"><span style="font-size:12px;color:var(--t3)">'+t('tokUsed')+':</span><div style="flex:1;height:4px;background:rgba(255,255,255,.08);border-radius:4px;overflow:hidden"><div style="height:100%;border-radius:4px;width:'+pct+'%;background:'+fillColor+'"></div></div><span style="font-size:12px;color:var(--t2)">'+S.tokenUsed.toLocaleString()+'</span><div class="chip" style="padding:4px 10px;font-size:12px" onclick="S.tokenUsed=0;localStorage.removeItem(\'tokenUsed\');toast(\'已重置\');closeModal()">重置</div></div>'+
...
window.presetAI=function(p,el){ $$('#preset-chips .chip').forEach(function(c){ c.classList.remove('on'); }); el.classList.add('on'); var epEl=$('#cfg-ep'),mdEl=$('#cfg-model'); if(p==='openai'&&epEl&&mdEl){ epEl.value='https://api.openai.com/v1/chat/completions'; mdEl.value='gpt-4o-mini'; } if(p==='poe'&&epEl&&mdEl){ epEl.value='https://api.poe.com/v1/chat/completions'; mdEl.value='GPT-4o-mini'; } };
...
function renderSet(){
  ...
  var LANG_LABEL={'zh-CN':'简','zh-TW':'繁','en':'EN'};
  var langChips=['zh-CN','zh-TW','en'].map(function(l){ return '<div class="chip '+(S.lang===l?'on':'')+'" style="font-weight:600" onclick="setLang(\''+l+'\')">'+LANG_LABEL[l]+'</div>'; }).join('');
  ...
  Object.entries(THEMES).forEach(function(entry){
    ...
    themeSwatches+='<div class="theme-swatch'+(S.theme===key?' on':'')+'" style="background:'+th.swatch+'" title="'+th.name+'" onclick="applyTheme(\''+key+'\');renderSet()"></div>';
  });
  themeSwatches+='</div>';
  ...
  var loginSection='';
  if(S.uid){
    loginSection='<div class="sec"><div class="sec-ttl">'+t('loginTitle')+'</div>'+
      '<div class="list"><div class="lr" style="cursor:default">'+
        (S.userPhoto?'<img src="'+escHtml(S.userPhoto)+'" style="width:34px;height:34px;border-radius:50%;flex-shrink:0">':'<div class="av" style="background:var(--blue)">'+ic('user',16)+'</div>')+
        '<div style="flex:1"><div style="font-size:14px;font-weight:600">'+escHtml(S.userEmail||'')+'</div></div>'+
      '</div>'+
    ...
    loginSection='<div class="sec"><div class="sec-ttl">'+t('loginTitle')+'</div>'+
      '<div class="list">'+
        '<div class="login-row" onclick="signInGoogle()">'+
          '<div class="login-logo" style="background:#fff">'+ic('google',20)+'</div>'+
          '<span style="font-size:15px;font-weight:500">'+t('loginGoogle')+'</span>'+
  ...
  if(S.localTrips.length>0){
    var histItems=S.localTrips.map(function(tr){
      return '<div class="lr" onclick="enterTrip(\''+tr.code+'\')">'+
        '<div style="flex:1"><div style="font-size:15px;font-weight:600">'+escHtml(tr.name||'—')+'</div>'+
        '<div class="hist-code">'+escHtml(tr.code)+'</div></div>'+
        '<span class="lr-val">'+escHtml(tr.dates||'')+'</span>'+
...
window.showMemberEdit=function(id){
  ...
  showModal(
    ...
    '</div>'+
    '<div class="inp-lbl">'+t('editNickname')+'</div>'+
    '<input class="inp" id="mem-name" value="'+escHtml(m.name)+'" style="margin-bottom:14px">'+
...
window.changeMemberAvatar=function(id){
  ...
  inp.onchange=function(){
    ...
    rd.onload=function(e){
      try{
        // Resize to max 120x120 to save space
        var img=new Image(); img.onload=function(){
          var canvas=document.createElement('canvas'); var size=Math.min(img.width,img.height,120); canvas.width=size; canvas.height=size;
          var ctx=canvas.getContext('2d'); ctx.drawImage(img,(img.width-size)/2,(img.height-size)/2,size,size,0,0,size,size);
...
window.submitMemberEdit=async function(id){
  ...
  S.members[id].name=name;
  if(id===S.memberId){ S.memberName=name; localStorage.setItem('memberName',name); }
...
window.submitAddMember=async function(){
  ...
  var id='u_'+Date.now(); var used=Object.values(S.members).map(function(m){ return m.color; }); var color=COLORS.find(function(c){ return used.indexOf(c)<0; })||COLORS[0];
