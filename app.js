...
var fbApp, db;
if(fbReady()){
  try{ fbApp=initializeApp(FB_CFG); db=getFirestore(fbApp); console.info('[Travoo] Firebase connected'); }
  catch(e){ console.warn('[FB]',e.message); }
}

// ── DEVICE ID (replaces login — zero config) ──────────────────
if(!localStorage.getItem('deviceId')){
  var _uid='dev_'+Math.random().toString(36).substr(2,9)+Date.now().toString(36);
  localStorage.setItem('deviceId',_uid);
}
var DEVICE_ID = localStorage.getItem('deviceId');

// ── THEMES ───────────────────────────────────────────────────
const THEMES = {
  dark:   {name:'Dark',   bg:'linear-gradient(158deg,#1a1610 0%,#111318 52%,#13100f 100%)',accent:'#ffffff',swatch:'#333'},
...
// FIX #3: exposed to window so onclick can find it
...
const WMO = {
  0:['☀️','晴朗','晴朗','Clear sky'],1:['🌤️','基本晴','基本晴','Mainly clear'],
  2:['⛅','部分多云','部分多雲','Partly cloudy'],3:['☁️','阴天','陰天','Overcast'],
  45:['🌫️','有雾','有霧','Fog'],48:['🌫️','冻雾','凍霧','Icy fog'],
  51:['🌦️','小毛雨','細雨','Light drizzle'],61:['🌧️','小雨','小雨','Slight rain'],
  63:['🌧️','中雨','中雨','Moderate rain'],65:['🌧️','大雨','大雨','Heavy rain'],
  71:['❄️','小雪','小雪','Slight snow'],73:['❄️','中雪','中雪','Moderate snow'],
  75:['❄️','大雪','大雪','Heavy snow'],80:['🌦️','阵雨','陣雨','Showers'],
  85:['❄️','阵雪','陣雪','Snow showers'],95:['⛈️','雷阵雨','雷陣雨','Thunderstorm'],
  96:['⛈️','雷冰雹','雷冰雹','Thunderstorm+hail'],
};
function wmoInfo(code){ return WMO[code]||WMO[0]; }

function getClothingRecs(tMax,tMin,precipProb,windSpeed){
  var avg=(tMax+tMin)/2, recs=[], en=S.lang==='en', tw=S.lang==='zh-TW';
  if(en){
    if(avg<0)    recs=['🧥 Heavy down jacket','🧤 Gloves','🧣 Scarf','👢 Winter boots'];
    else if(avg<6)  recs=['🧥 Down jacket','🧤 Gloves','🧣 Scarf','👢 Warm boots'];
    else if(avg<12) recs=['🧥 Heavy coat','🧥 Sweater/fleece','👖 Long pants'];
    else if(avg<18) recs=['🧥 Light jacket','👕 Long sleeve','👟 Sneakers'];
    else if(avg<24) recs=['👕 T-shirt','🧥 Jacket for evenings','👟 Sneakers'];
    else            recs=['👔 Light top/singlet','🩳 Shorts','🕶️ Sunglasses','🧴 SPF50'];
    if(precipProb>40) recs.push('☂️ Umbrella');
    if(windSpeed>25)  recs.push('🌬️ Windbreaker');
    if(avg>32) recs.push('💧 Stay hydrated!');
  } else {
    if(avg<0)    recs=['🧥 厚羽绒服','🧤 手套','🧣 围巾','👢 保暖靴','🎿 防滑装备'];
    else if(avg<6)  recs=['🧥 羽绒服','🧤 手套','🧣 围巾','👢 保暖靴'];
    else if(avg<12) recs=['🧥 厚外套','🧥 毛衣','👖 厚长裤'];
    else if(avg<18) recs=['🧥 薄外套','👕 长袖','👟 运动鞋'];
    else if(avg<24) recs=['👕 T恤','🧥 晚上带外套','👟 舒适鞋'];
    else            recs=['👔 背心/短袖','🩳 短裤','🕶️ 太阳镜','🧴 防晒霜SPF50'];
    if(precipProb>40) recs.push('☂️ 带雨伞');
    if(windSpeed>25)  recs.push('🌬️ 防风外套');
    if(avg>32) recs.push('💧 多补充水分');
    if(avg<-5) recs.push('🔋 备用充电宝（低温耗电快）');
  }
  return recs;
}
...
async function fetchWeather(){
  ...
  try{
    ...
    localStorage.setItem('wxCache',JSON.stringify({data:S.weather,ts:Date.now(),lat:S.geo.lat,lon:S.geo.lon}));
...
function renderWeatherWidget(){
  ...
  if(!S.geo){
    return '<div class="wx-widget" onclick="reqGeoWeather()" style="padding:12px 14px;display:flex;align-items:center;gap:12px">'+
      ...
      '<div style="flex:1"><div style="font-size:13px;font-weight:600">'+(en?'Enable location for weather':'开启位置权限以查看天气')+'</div>'+
      '<div style="font-size:11px;color:var(--t3)">'+(en?'Clothing tips & forecast':'穿搭建议 · 3天预报')+'</div></div>'+
  ...
  if(!S.weather){
    return '<div class="wx-widget" style="padding:12px 14px;display:flex;align-items:center;gap:10px">'+
      '<div style="font-size:22px">🌡️</div><div style="font-size:13px;color:var(--t2)">'+(en?'Loading weather…':'天气加载中…')+'</div>'+
    '</div>';
  }
  var cur=S.weather.current, daily=S.weather.daily;
  var wi=wmoInfo(cur.weathercode);
  var temp=Math.round(cur.temperature_2m), hum=cur.relative_humidity_2m, wind=Math.round(cur.windspeed_10m);
  var desc=en?wi[3]:S.lang==='zh-TW'?wi[2]:wi[1];
  var fHtml='';
  ...
  for(var i=0;i<Math.min(3,daily.time.length);i++){
    ...
    var di=wmoInfo(daily.weathercode[i]);
    ...
    fHtml+='<div style="flex:1;text-align:center">'+
      ...
      '<div style="font-size:16px;margin-bottom:1px">'+di[0]+'</div>'+
      '<div style="font-size:11px;font-weight:600">'+Math.round(daily.temperature_2m_max[i])+'°</div>'+
      '<div style="font-size:10px;color:var(--t3)">'+Math.round(daily.temperature_2m_min[i])+'°</div>'+
      (prec>20?'<div style="font-size:10px;color:#60a0ff">'+prec+'%</div>':'')+'</div>';
  }
  var tMax=daily.temperature_2m_max[0], tMin=daily.temperature_2m_min[0];
  var prec0=daily.precipitation_probability_mean[0]||0;
  var clothes=getClothingRecs(tMax,tMin,prec0,wind).slice(0,4);
  var cHtml=clothes.map(function(c){ return '<span class="wx-pill">'+c+'</span>'; }).join('');
  return '<div class="wx-widget" onclick="showWeatherDetail()">'+
    '<div class="wx-row">'+
      '<div style="flex:1">'+
        '<div style="font-size:11px;color:var(--t3);margin-bottom:3px">'+(en?'Local Weather':'当地天气')+'</div>'+
        '<div style="display:flex;align-items:center;gap:8px">'+
          '<div style="font-size:30px">'+wi[0]+'</div>'+
          '<div><div style="font-size:22px;font-weight:700;line-height:1">'+temp+'°C</div>'+
          '<div style="font-size:11px;color:var(--t2)">'+desc+' · 💧'+hum+'%</div></div>'+
        '</div>'+
      '</div>'+
      '<div style="display:flex;gap:4px">'+fHtml+'</div>'+
    '</div>'+
    (cHtml?'<div class="wx-clothes">'+cHtml+'</div>':'')+
  '</div>';
}
window.reqGeoWeather=function(){ requestGeo(); setTimeout(function(){ if(S.geo) fetchWeather(); else toast(S.lang==='en'?'Please allow location':'请允许位置权限'); renderHome(); },1200); };
window.showWeatherDetail=function(){
  ...
  for(var i=0;i<daily.time.length;i++){
    ...
    fRows+='<div class="lr" style="cursor:default">'+
      '<span style="font-size:18px">'+di[0]+'</span>'+
  ...
  var cDiv=clothes.map(function(c){ return '<div style="padding:6px 11px;background:rgba(255,255,255,.07);border-radius:10px;font-size:13px">'+c+'</div>'; }).join('');
  showModal(
    '<div class="sh"></div>'+
    '<div style="text-align:center;padding:8px 0 16px">'+
      '<div style="font-size:48px">'+wi[0]+'</div>'+
      '<div style="font-size:34px;font-weight:800;margin-bottom:3px">'+temp+'°C</div>'+
      '<div style="font-size:14px;color:var(--t2)">'+desc+'</div>'+
    ...
    '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:4px">'+cDiv+'</div>'
  );
};

// ── XHS POOL — real search topics (#7 fix) ────────────────────
const XHS_POOL = [
  {kw:'首尔美食弘大必吃',      label:'弘大美食',     desc:'炸鸡年糕血肠 → 搜索小红书'},
  {kw:'首尔东大门购物攻略',    label:'东大门购物',   desc:'批发价拿货技巧 → 搜索'},
  {kw:'N首尔塔夜景拍照',       label:'首尔塔夜景',   desc:'最佳机位时间 → 搜索'},
  {kw:'首尔圣水洞咖啡厅',      label:'圣水洞咖啡',   desc:'工业风网红打卡 → 搜索'},
  {kw:'济州岛自驾租车攻略',    label:'济州自驾',     desc:'3天路线全攻略 → 搜索'},
  {kw:'仁川机场免税店必买',    label:'机场免税',     desc:'性价比清单 → 搜索'},
  {kw:'韩国便利店必买零食',    label:'便利店好物',   desc:'GS25 CU 7-11 → 搜索'},
  {kw:'首尔汉江公园野餐',      label:'汉江野餐',     desc:'位置外卖穿搭 → 搜索'},
  {kw:'景福宫韩服体验',        label:'韩服体验',     desc:'价格造型拍照位 → 搜索'},
  {kw:'首尔地铁攻略T-money',   label:'地铁攻略',     desc:'新手必看指南 → 搜索'},
  {kw:'韩国换汇明洞最划算',    label:'换汇攻略',     desc:'明洞vs机场vs银行 → 搜索'},
  {kw:'首尔梨泰院美食推荐',    label:'梨泰院美食',   desc:'异国料理大集合 → 搜索'},
  {kw:'首尔汗蒸幕体验攻略',    label:'汗蒸幕',       desc:'礼仪流程推荐 → 搜索'},
  {kw:'首尔江南COEX购物',      label:'江南探店',     desc:'星空图书馆周边 → 搜索'},
  {kw:'韩国乐天玛特必买清单',  label:'乐天玛特',     desc:'零食护肤品攻略 → 搜索'},
  {kw:'首尔益善洞古街咖啡',    label:'益善洞',       desc:'韩服+传统小吃 → 搜索'},
  {kw:'韩国KTX高铁订票',       label:'KTX订票',      desc:'提前购省钱方法 → 搜索'},
  {kw:'首尔北村韩屋村拍照',    label:'北村韩屋',     desc:'最美机位时间段 → 搜索'},
  {kw:'韩国平价美妆药妆店',    label:'韩国美妆',     desc:'药妆必买清单 → 搜索'},
  {kw:'首尔烤肉推荐五花肉',    label:'首尔烤肉',     desc:'人均价格门店 → 搜索'},
  {kw:'韩国特产伴手礼清单',    label:'伴手礼',       desc:'回国必带礼品 → 搜索'},
  {kw:'首尔旅行穿搭秋冬',      label:'旅行穿搭',     desc:'实用穿搭指南 → 搜索'},
  {kw:'首尔乐天世界攻略',      label:'乐天世界',     desc:'门票省钱必玩 → 搜索'},
  {kw:'韩国整形医院参考',      label:'医美参考',     desc:'正规机构选择 → 搜索'},
];
var _xhsOffset=0;
...
const LANGS = {
  'zh-CN':{
    ...
    themes:'主题颜色',loginTitle:'设备同步',loginGoogle:'Google 登入',
    ...
    deviceSync:'行程码同步',deviceSyncDesc:'你的数据保存在本设备及 Firebase 云端',
    deviceId:'设备 ID',
  ...
  'zh-TW':{
    ...
    themes:'主題顏色',loginTitle:'設備同步',loginGoogle:'Google 登入',
    ...
    deviceSync:'行程碼同步',deviceSyncDesc:'你的資料保存在本設備及 Firebase 雲端',
    deviceId:'設備 ID',
  ...
  'en':{
    ...
    qa:'Quick Actions',smRec:'Smart Tips',xhs:'Xiaohongshu Search',
    ...
    themes:'Theme Color',loginTitle:'Device Sync',loginGoogle:'Sign in with Google',
...
const IC = {
  ...
  palette:'<svg viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2a10 10 0 100 20c1.1 0 2-.9 2-2v-.5c0-.83.67-1.5 1.5-1.5H17a3 3 0 003-3 8 8 0 00-8-8z"/><circle cx="7.5" cy="10.5" r="1"/><circle cx="10" cy="7" r="1"/><circle cx="14" cy="7" r="1"/><circle cx="16.5" cy="10" r="1"/></svg>',
...
function sysPrompt(){
  ...
  return 'You are Travoo Butler for trip "'+((S.trip&&S.trip.name)||'Trip')+'". Today: '+today()+(td?' - '+td.title:'')+'. Members: '+Object.values(S.members).map(function(m){ return m.name; }).join(', ')+'. Base currency: '+lc.name+'. Reply in user\'s language. Be concise and practical.';
}
...
// FIX #7: real XHS search
window.openXHS=function(kw){ openApp('xiaohongshu',encodeURIComponent(kw||'首尔旅行')); };
...
function renderHome(){
  ...
  // FIX #7: XHS as real search cards
  ...
  var xhsHtml='<div style="margin-bottom:18px">'+
    '<div style="display:flex;align-items:center;padding:0 16px;margin-bottom:8px">'+
      ...
      '<div onclick="refreshXHS()" style="display:flex;align-items:center;gap:4px;font-size:12px;color:var(--t3);cursor:pointer;padding:4px 8px;border-radius:10px;transition:background .15s" onmousedown="this.style.background=\'rgba(255,255,255,.06)\'" ontouchstart="this.style.background=\'rgba(255,255,255,.06)\'">'+ic('refresh',12)+' '+t('xhsRefresh')+'</div>'+
  ...
  xhsRecs.forEach(function(r){
    ...
    xhsHtml+='<div class="xhs-card" onclick="openXHS(\''+kwEnc+'\')">'+
      '<div class="xhs-card-top">'+
        ...
        '<div class="xhs-card-kw">'+escHtml(r.label)+'</div>'+
      '</div>'+
      '<div class="xhs-body">'+
        '<div class="xhs-ttl">'+escHtml(r.desc.replace(' → 搜索','').replace(' → 搜索小红书',''))+'</div>'+
        '<div class="xhs-go">🔍 点击在小红书搜索</div>'+
      '</div></div>';
  });
  xhsHtml+='</div></div>';
  ...
  var rateBar='<div style="margin:0 16px 14px;padding:10px 14px;background:var(--g1);border:1px solid var(--gb);border-radius:var(--r2);display:flex;align-items:center;gap:10px" onclick="switchTab(\'set\')">'+
...
window.doFetchRates=async function(){ toast('获取汇率中...',0); var ok=await fetchRates(); toast(ok?'汇率已更新':'汇率获取失败'); renderHome(); };
function buildSmartRecs(todayDay,h){
  ...
  if(S.geo) recs.push({type:'位置感知',title:'已获取位置',desc:'高德可规划最优路线',action:"openApp('maps')"});
...
function renderItin(){
  ...
  days.forEach(function(day,di){
    ...
    pagesHtml+='<div class="itin-page" id="ipg-'+di+'">'+
      '<div class="day-hdr">'+
        '<div class="day-hdr-title" onclick="editDayTitle('+di+')">'+escHtml(day.title)+' <span style="opacity:.25;font-size:12px">✏️</span></div>'+
...
function renderActCard(item){
  ...
  var appBtns=''; if(item.apps&&item.apps.length) item.apps.forEach(function(a){ if(!APPS[a]) return; var ico={didi:'car',maps:'map',ctrip:'plane','12306':'train',dianping:'food',xiaohongshu:'xhs'}[a]||'globe'; appBtns+='<div class="act-app" onclick="event.stopPropagation();openApp(\''+a+'\')">'+ic(ico,12)+' '+escHtml(APPS[a].label)+'</div>'; });
...
window.importFromImage=function(){
  ...
  inp.onchange=async function(){
    var file=inp.files[0]; if(!file) return; closeModal(); showLoad();
    var reader=new FileReader();
    reader.onload=async function(e){ try{ var b64=e.target.result; var days=await importItineraryFromImage(b64); if(!days||!days.length) throw new Error('无法识别'); await fbSaveDays(days); _updateTripDates(days); hideLoad(); renderItin(); toast(t('importOk')+'，共'+days.length+'天'); }catch(err){ hideLoad(); toast(t('importFail')+'：'+err.message); } };
...
window.showExpDetail=function(id){
  ...
  showModal('<div class="sh"></div><div style="font-size:20px;font-weight:700;margin-bottom:4px">'+escHtml(e.description||t('other'))+'</div><div style="font-size:36px;font-weight:800;color:var(--red);margin:10px 0">'+amtDisplay+'</div><div class="list" style="margin-bottom:14px"><div class="lr" style="cursor:default"><span class="lr-lbl">'+t('paidBy')+'</span><span class="lr-val">'+escHtml(memberName(e.paidBy))+'</span></div><div class="lr" style="cursor:default"><span class="lr-lbl">'+t('splitW')+'</span><span class="lr-val">'+escHtml(splitNames)+'</span></div><div class="lr" style="cursor:default"><span class="lr-lbl">'+t('date')+'</span><span class="lr-val">'+escHtml(e.date||'')+'</span></div>'+convRow+'</div><button class="btn btn-d btn-full" onclick="fbDelExpense(\''+e.id+'\');closeModal();toast(t(\'deleted\'))">'+ic('trash',16)+' '+t('del')+'</button>');
};
window.showAddExpenseModal=function(prefill){
  ...
  showModal('<div class="sh"></div><div style="font-size:18px;font-weight:700;margin-bottom:14px">'+t('addExpense')+'</div>'+
    ...
    '<div style="display:flex;gap:8px;margin-bottom:6px">'+
      ...
      '<div style="min-width:120px"><div class="inp-lbl">'+t('expCurrency')+'</div><select class="inp" id="ex-cur" onchange="updateExpConv()">'+curOpts+'</select></div>'+
    '</div>'+
    '<div id="exp-conv-hint" style="font-size:12px;color:var(--t3);margin-bottom:10px;padding-left:2px;min-height:16px">'+escHtml(rateHint)+'</div>'+
...
window.updateExpConv=function(){
  ...
  if(cur!==S.baseCurrency&&amt>0&&Object.keys(S.rates).length>0){ hint.textContent='≈ '+fmtCurrency(toBase(amt,cur),S.baseCurrency); }
  else { hint.textContent=Object.keys(S.rates).length>0?'1 '+S.localCurrency+' = '+fmtCurrency(getRate(S.localCurrency,S.baseCurrency),S.baseCurrency):''; }
};
...
// FIX #9: Chat clear with 2-step confirmation + red
window.confirmClearChat=function(){
  showModal(
    '<div class="sh"></div>'+
    '<div style="font-size:22px;text-align:center;margin-bottom:8px">🗑️</div>'+
    '<div style="font-size:18px;font-weight:700;text-align:center;margin-bottom:8px">'+t('confirmClearChat')+'</div>'+
    '<div style="font-size:14px;color:var(--t2);text-align:center;margin-bottom:22px">'+t('confirmClearChatSub')+'</div>'+
    '<button class="btn btn-d btn-full" onclick="S.chatHistory=[];toast(t(\'chatCleared\'));closeModal()" style="margin-bottom:10px;font-size:16px">'+
      ic('trash',16)+' '+t('clearChatConfirmBtn')+
    '</button>'+
    '<button class="btn btn-g btn-full" onclick="closeModal()">'+t('cancel')+'</button>'
  );
};
...
// ── SETTINGS ──────────────────────────────────────────────────
// FIX #2: appearance in a modal
window.showAppearanceModal=function(){
  var LANG_LABEL={'zh-CN':'简','zh-TW':'繁','en':'EN'};
  var langChips=['zh-CN','zh-TW','en'].map(function(l){ return '<div class="chip '+(S.lang===l?'on':'')+'" style="font-weight:600" onclick="setLang(\''+l+'\');document.querySelectorAll(\'#ap-lang .chip\').forEach(function(c){c.classList.remove(\'on\')});this.classList.add(\'on\')">'+LANG_LABEL[l]+'</div>'; }).join('');
  // FIX #3: theme swatches call window.applyTheme
  var themeSwatches='<div class="theme-grid">';
  Object.entries(THEMES).forEach(function(entry){ var k=entry[0],th=entry[1]; themeSwatches+='<div class="theme-swatch'+(S.theme===k?' on':'')+'" style="background:'+th.swatch+'" title="'+th.name+'" onclick="window.applyTheme(\''+k+'\');document.querySelectorAll(\'.theme-swatch\').forEach(function(s){s.classList.remove(\'on\')});this.classList.add(\'on\')"></div>'; });
  themeSwatches+='</div>';
  showModal(
    '<div class="sh"></div>'+
    '<div style="font-size:18px;font-weight:700;margin-bottom:18px">'+t('appearance')+'</div>'+
...
function renderSet(){
  ...
  // FIX #8: History without trip code
  var histSection='';
  if(S.localTrips.length>0){
    var histItems=S.localTrips.map(function(tr){ return '<div class="lr" onclick="enterTrip(\''+tr.code+'\')"><div style="flex:1"><div style="font-size:15px;font-weight:600">'+escHtml(tr.name||'—')+'</div><div style="font-size:12px;color:var(--t3)">'+escHtml(tr.dates||'—')+'</div></div><span class="lr-chev">'+ic('chev',16)+'</span></div>'; }).join('');
    histSection='<div class="sec"><div class="sec-ttl">'+t('history')+'</div><div class="list">'+histItems+'</div></div>';
  }

  // Device sync section (replaces Google login)
  var shortId=DEVICE_ID.substring(0,12)+'…';
  var deviceSection='<div class="sec"><div class="sec-ttl">'+t('loginTitle')+'</div><div class="list">'+
    '<div class="lr" style="cursor:default"><span style="font-size:18px">📱</span><div style="flex:1"><div class="lr-lbl" style="font-size:14px">'+t('deviceSync')+'</div><div style="font-size:11px;color:var(--t3)">'+t('deviceSyncDesc')+'</div></div></div>'+
    '<div class="lr" style="cursor:default"><span class="lr-lbl" style="font-size:13px">'+t('deviceId')+'</span><span class="lr-val" style="font-size:11px;font-family:monospace">'+escHtml(shortId)+'</span></div>'+
  '</div></div>';
  ...
  v.innerHTML=
    ...
    '<div class="scroller"><div style="height:12px"></div>'+
      ...
      '<div class="sec"><div class="list"><div class="lr" onclick="showAppearanceModal()"><span style="font-size:18px">🎨</span><div style="flex:1"><div class="lr-lbl">'+t('appearance')+'</div><div style="font-size:12px;color:var(--t3)">'+t('appearanceDesc')+'</div></div><span class="lr-chev">'+ic('chev',16)+'</span></div></div></div>'+
      // Currency
      '<div class="sec"><div class="sec-ttl">'+t('currency')+'</div>'+
        ...
        '<div style="padding:10px 12px;background:var(--g1);border:1px solid var(--gb);border-radius:var(--r2);display:flex;align-items:center;gap:10px"><div style="flex:1"><div style="font-size:13px;font-weight:600">'+escHtml(rateStr)+'</div><div style="font-size:10px;color:var(--t3);margin-top:2px">'+t('rateDate')+': '+escHtml(S.fxDate?S.fxDate.substring(0,16):'—')+'</div></div><button class="btn btn-g" style="padding:7px 12px;font-size:13px" onclick="doFetchRatesSet()">'+ic('refresh',13)+' '+t('refreshRate')+'</button></div>'+
      '</div>'+
      ...
      '<div class="sec"><div class="sec-ttl">'+t('aiCfg')+'</div><div class="list">'+
        ...
        // FIX #9: clear chat → confirmation modal
...
window.changeMemberAvatar=function(id){
  ...
  inp.onchange=function(){
    ...
    rd.onload=function(e){
      try{ var img=new Image(); img.onload=function(){ var canvas=document.createElement('canvas'),sz=Math.min(img.width,img.height,120); canvas.width=sz; canvas.height=sz; var ctx=canvas.getContext('2d'); ctx.drawImage(img,(img.width-sz)/2,(img.height-sz)/2,sz,sz,0,0,sz,sz); var b64=canvas.toDataURL('image/jpeg',0.7); S.avatars[id]=b64; localStorage.setItem('memberAvatars',JSON.stringify(S.avatars)); closeModal(); renderSet(); toast(t('wallUpdated')); }; img.src=e.target.result; }catch(err){ toast(t('imgTooLarge')); }
    };
    rd.readAsDataURL(f);
  };
  inp.click();
...
async function init(){
  ...
  renderApp();
  // Auto-fetch weather if geo is cached or available
